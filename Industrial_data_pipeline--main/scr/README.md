# Data_Pipeline 
This repo demonstrates my apporach for **reproducible, scalable, and production-oriented data pipelines**. This project mainly focuses on transforming raw, noisy data into analytical ready output.

# Key objectives
- To build a modular and maintainable data pipeline
- To handle real-world imperfections
- To ensure reproducibility and transparency
- To have reliable analytics and decision support
...
- Description of Columns,
     ...
     - ingredients : gives the list of ingredients required to prepare the particular dish
...
## Project Structure
- input folder : contains all the .json files
...
- scripts : contains the actual scripts where the folder consists of following .py files,
...
3) In the data_preprocessing.py file, .json files are converted to .csv file and created the input_csv folder to store all the converted .csv files by merging them for further processing
...
6) Outliers are capped using  IQR method/Winsorization method, as out of 1047 rows there are 231 outliers for cookTime and prepTime columns which is 22.06% and hence is not suggestible to drop off the columns with outliers 
...
8) Ingredients are stored in list format in the dataset, hence parsing is required
9) The dataset is then classified into easy, medium and hard
...
12) Normalization method was not that suitable as the time frame was getting converted between 0-1 range which is not suitable
13) manin.py files calls all the functions that are defined in other .py files
14) .md file is used for documentation 

## Output 
- The final output is stored in output folder 
